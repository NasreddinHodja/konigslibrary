...
State management uses a =createReaderServices()= factory that wires together a Svelte 5 =$state= object with the command registry, event bus, viewer registry, and plugin runner. No stores, no context API — one reactive service object owns all reader state and logic.
