...
- *Android app* — capacitor APK that reads from device storage or connects to a local server
...
The Android app connects to a running local server or reads directly from device storage. Build the APK with:
